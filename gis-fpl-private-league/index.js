...
function displayLeagueTableWithToggle(teamsToKeep = []) {
  ...
  const filteredAndReRankedTeams = leagueData.standings.results
    ...
    .map((team, index, arr) => {
      let currentRank = index === 0 || team.total < arr[index - 1].total ? index + 1 : arr[index - 1].rank;
      return { ...team,
        rank: currentRank
      };
    });
  ...
  const renderTable = (teams, isHiddenView) => {
    ...
    toggleButton.textContent = isHiddenView ? "Show Full League" : "Show Paid League";
    toggleButton.onclick = () => {
      renderTable(isHiddenView ? leagueData.standings.results : filteredAndReRankedTeams, !isHiddenView);
    };
